...
export default function DeleteRequestModal({
  ...
  const handleConfirmDelete = async () => {
    if (!projectId || !collectionId || !request._id) return;
    ...
    try {
      ...
      if (result) {
        setIsDeleting(false);
        onClose();
        if (onDeleted) onDeleted(request);
      }
    } catch (err) {
      setDeleteError(
        typeof err === "string"
      ...
      <div className="w-full max-w-md rounded-lg bg-[#FFFFFF] dark:bg-[#141416] border border-[#E6D2A5] dark:border-[#2C2C2E] p-6 shadow-xl space-y-4 animate-in zoom-in-95 duration-150">
        ...
        <div className="flex items-center justify-end gap-2.5 pt-2">
          <button
            ...
            onClick={onClose}
            disabled={isDeleting}
            className="px-3.5 py-1.5 rounded-md bg-[#FFFFFF] dark:bg-[#1C1C1F] hover:bg-[#F5E7C6] dark:hover:bg-[#2C2C2E] text-[#5C5C5C] dark:text-[#A1A1A6] border border-[#E6D2A5] dark:border-[#2C2C2E] text-xs font-medium transition-colors cursor-pointer"
          >
            Cancel
          </button>
